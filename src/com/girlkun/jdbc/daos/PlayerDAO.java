...
public class PlayerDAO {
    ...
    public static boolean createNewPlayer(int userId, String name, byte gender, int hair) {
        try {
            ...
            for (int i = 0; i < 120; i++) { // item tạo player 
//                if (i == 0) { //Capsule Vip
...
//                    opt.clear();
            ...
            int[] defaultShortcuts = gender == 0
                    ? new int[]{0, 1, 6, 9, 10, 20, 22, 24, 19, 28}
                    : gender == 1
                            ? new int[]{2, 3, 7, 11, 12, 17, 18, 26, 19, 28}
                    : new int[]{4, 5, 8, 13, 14, 21, 23, 25, 19, 28};

            // Gán 10 ô shortcut
            for (int i = 0; i < 10; i++) {
                dataArray.add(defaultShortcuts[i]);
            }
...
//                Logger.success("Total time save player " + player.name + " thành công! " + (System.currentTimeMillis() - st) + "\n");
    ...
    public static void setActive(Player player, boolean active) {
        try (Connection con = GirlkunDB.getConnection(); PreparedStatement ps = con.prepareStatement("UPDATE account SET active = ? WHERE id = ?")) {
            ps.setInt(1, active ? 1 : 0);
            ps.setInt(2, player.getSession().userId);
            ps.executeUpdate();
        } catch (Exception e) {
            Logger.logException(PlayerDAO.class, e, "Lỗi cập nhật active " + player.name);
        }
    }
