...
//        player.effectSkill.timeAnTroi = timeAnTroi;
    ...
    public void applyStun(Player caster, Player target, long startTime, int duration) {
        if (target.nPoint.khangTDHS) {
            Service.gI().sendThongBao(target, "Bạn đã kháng Thái Dương Hạ San!");
            return;
        }

        target.effectSkill.lastTimeStartStun = startTime;
        target.effectSkill.timeStun = duration;
        target.effectSkill.isStun = true;

        sendEffectPlayer(caster, target, TURN_ON_EFFECT, BLIND_EFFECT);
    }

    // Stun cho Player
    public void startStun(Player player, long lastTimeStartBlind, int timeBlind) {
        // System.out.println("Gọi stun cho " + player.name + " - KhangTDHS: " + player.nPoint.khangTDHS);
        if (player.nPoint.khangTDHS) {
            Service.getInstance().sendThongBao(player, "Bạn đã kháng Thái Dương Hạ San!");
            return;
        }
        if (player.effectSkill.isStun) {
            return; // Đã bị choáng rồi, không choáng tiếp
        }
...
// Stun cho Mob
    public void startStunMob(Player plUseSkill, Mob mobTarget, long lastTimeStartBlind, int timeBlind) {
        if (plUseSkill.nPoint.khangTDHS) {
            Service.gI().sendThongBao(plUseSkill, "Bạn đã kháng Thái Dương Hạ San!");
            return;
        }

        mobTarget.effectSkill.lastTimeStun = lastTimeStartBlind;
        mobTarget.effectSkill.timeStun = timeBlind;
        mobTarget.effectSkill.isStun = true;

        sendEffectMob(plUseSkill, mobTarget, TURN_ON_EFFECT, BLIND_EFFECT);
    }
    ...
    public void setStartHuytSao(Player player, int tiLeHP) {
        if (player.effectSkill == null) {
            player.effectSkill = new EffectSkill(player); // backup
        }
...
//        player.isbroly = 0;
    ...
    void removeMonkeyEffect(Player player) {
