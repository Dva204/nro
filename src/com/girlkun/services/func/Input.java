...
public class Input {
    ...
    public static final int KHUYEN_MAI_NAP = 620;
    ...
    public static final int TAITV_taixiu = 5166;
    public static final int XIUTV_taixiu = 5167;
        ...
        try {
            ...
            switch (player.iDMark.getTypeInput()) {
                ...
                case UseGold:
                    int Gold;
                    try {
                        Gold = Integer.parseInt(text[0]);
                        if (Gold <= 0) {
                            Service.gI().sendThongBao(player, "Số lượng không hợp lệ");
                            break;
                        }
                    } catch (Exception e) {
                        Service.gI().sendThongBao(player, "Input không hợp lệ");
                        break;
                    }
                    ...
                    long goldsum = 500000000L * Gold;

                    if (player.inventory.gold + goldsum > Inventory.LIMIT_GOLD) {
                        Service.gI().sendThongBao(player, "Số vàng vượt quá giới hạn 200 tỉ");
                        break;
                    }

                    player.inventory.gold += goldsum;
                ...
                case TAI_taixiu:
                    ...
                    break;
                case TAITV_taixiu:
                    int tvxiu1 = Integer.valueOf(text[0]);
                    Item tvtx1 = InventoryServiceNew.gI().findItemBag(player, 457);
                    try {
                        if (tvxiu1 >= 10 && tvxiu1 <= 100000) {
                            if (tvtx1.quantity >= tvxiu1) {
                                tvtx1.quantity -= tvxiu1;
                                if (tvtx1.quantity <= 0) {
                                    InventoryServiceNew.gI().removeItemBag(player, tvtx1);
                                }
                                player.tvTai += tvxiu1;
                                TaiXiu.gI().tvTai += tvxiu1;
                                Service.gI().sendThongBao(player, "Bạn đã đặt " + Util.format(tvxiu1) + " Thỏi vàng vào TÀI");
                                TaiXiu.gI().addPlayerTai(player);
                                InventoryServiceNew.gI().sendItemBags(player);
                                Service.getInstance().sendMoney(player);
                                PlayerDAO.updatePlayer(player);
                            ...
                            Service.gI().sendThongBao(player, "Cược ít nhất 10 - nhiều nhất 100.000 Hồng ngọc");
                        }
                    } catch (Exception e) {
                        e.printStackTrace();
                        Service.gI().sendThongBao(player, "Lỗi.");
                ...
                case XIU_taixiu:
                    ...
                    break;
                case XIUTV_taixiu:
                    int tvxiu2 = Integer.valueOf(text[0]);
                    Item tvtx2 = null;
                    try {
                        tvtx2 = InventoryServiceNew.gI().findItemBagByTemp(player, 457);
                    } catch (Exception e) {

                    }
                    try {
                        if (tvxiu2 >= 10 && tvxiu2 <= 100000) {
                            if (tvtx2.quantity >= tvxiu2) {
                                tvtx2.quantity -= tvxiu2;
                                if (tvtx2.quantity <= 0) {
                                    InventoryServiceNew.gI().removeItemBag(player, tvtx2);
                                }
                                player.tvXiu += tvxiu2;
                                TaiXiu.gI().goldXiu += tvxiu2;
                                Service.gI().sendThongBao(player, "Bạn đã đặt " + Util.format(tvxiu2) + " Thỏi vàng vào XỈU");
                                TaiXiu.gI().addPlayerXiu(player);
                                InventoryServiceNew.gI().sendItemBags(player);
                                Service.getInstance().sendMoney(player);
                                PlayerDAO.updatePlayer(player);
                ...
                case NAP_COIN: {
                    ...
                    if (text.length > 2) {
                        try {
                            khuyenMai = Integer.parseInt(text[2]);
                            if (khuyenMai <= 0) {
                                khuyenMai = 1;
                            }
                        } catch (Exception e) {
                    ...
                    int vndKhuyenMai = vnd * khuyenMai;

                    Player pl = Client.gI().getPlayer(name);
                    if (pl != null) {
                        pl.getSession().vnd += vndKhuyenMai;

                        PreparedStatement ps = null;
                        try (Connection con = GirlkunDB.getConnection()) {
                            ps = con.prepareStatement("UPDATE account SET vnd = (vnd + ?), tongnap = (tongnap + ?) WHERE id = ?");
                            ps.setInt(1, vndKhuyenMai); // coin tăng theo khuyến mãi
                            ps.setInt(2, vndKhuyenMai); // tổng nạp cũng tăng theo khuyến mãi
                        ...
                        Service.getInstance().sendThongBao(player, "Đã nạp " + vndKhuyenMai + " VNĐ (KM x" + khuyenMai + ") cho " + pl.name);
                    } else {
                ...
                case FIND_PLAYER:
                    ...
                    if (pl != null) {
                        ...
                        NpcService.gI().createMenuConMeo(player, ConstNpc.MENU_FIND_PLAYER, 12639, "|7|[dva]\n"
...
//                                +") where id = " + player.id); 
    ...
    private void duahau(Player player, String s) {
        ...
        Item ngocboi = InventoryServiceNew.gI().findItemBag(player, 569);
    ...
    private void bonghoa(Player player, String s) {
        ...
        if (player.inventory.event >= 100 * sl) {
            Item capsule = ItemService.gI().createNewItem((short) 1946, sl);
    ...
    public void TAITV_taixiu(Player pl) {
        createForm(pl, TAITV_taixiu, "Chọn số thỏi vàng đặt Tài", new SubInput("Số Thỏi vàng cược", ANY));//????
    }
    ...
    public void XIUTV_taixiu(Player pl) {
        createForm(pl, XIUTV_taixiu, "Chọn số thỏi vàng đặt Xỉu", new SubInput("Số Thỏi vàng cược", ANY));//????
    }
    ...
    public void createFormGiftCode(Player pl) {
        createForm(pl, GIFT_CODE, "GIFTCODE", new SubInput("MÃ QUÀ TẶNG", ANY));
    }
...
//    }
    ...
    public void addItemGiftCodeToPlayer(Player p, final String giftcode) {
        try {
            ...
            if (!red.first()) {
                NpcService.gI().createTutorial(p, 30274, "Mã quà tặng không tồn tại hoặc đã được sử dụng");
                return;
            }

            // Kiểm tra hết hạn
            final Timestamp expiredTimestamp = red.getTimestamp("expired");
            if (expiredTimestamp != null && new Date().getTime() > expiredTimestamp.getTime()) {
                Service.gI().sendThongBaoOK(p, "GiftCode đã hết hạn sử dụng.");
                return;
            }
            ...
            // Global itemoption (fallback nếu item không có options riêng)
            JSONArray globalOption = null;
            String optionStr = red.getString("itemoption");
            if (optionStr != null && !optionStr.isEmpty()) {
                globalOption = (JSONArray) JSONValue.parseWithException(optionStr);
            }
            ...
            // Kiểm tra đã dùng (type == 1: mỗi tài khoản chỉ dùng 1 lần)
            if (type == 1) {
                for (int i = 0; i < listUser.size(); ++i) {
                    if (Integer.parseInt(listUser.get(i).toString()) == p.id) {
            ...
            for (int i = 0; i < listItem.size(); ++i) {
                ...
                if (idItem == -1) {
                    ...
                    // Ưu tiên options riêng của item trong listItem JSON
                    // Nếu có key "options" → dùng options riêng (kể cả rỗng = không có option)
                    // Nếu không có key "options" → fallback về global itemoption
                    if (item.containsKey("options")) {
                        JSONArray itemOptions = (JSONArray) item.get("options");
                        if (itemOptions != null) {
                            for (int u = 0; u < itemOptions.size(); u++) {
                                JSONObject opt = (JSONObject) itemOptions.get(u);
                                itemGiftTemplate.itemOptions.add(new Item.ItemOption(
                                        Integer.parseInt(opt.get("id").toString()),
                                        Integer.parseInt(opt.get("param").toString())
                                ));
                            }
                        }
                    } else if (globalOption != null) {
                        // Không có key "options" → dùng global itemoption (tương thích ngược dữ liệu cũ)
                        for (int u = 0; u < globalOption.size(); u++) {
                            JSONObject opt = (JSONObject) globalOption.get(u);
                            itemGiftTemplate.itemOptions.add(new Item.ItemOption(
                                    Integer.parseInt(opt.get("id").toString()),
                                    Integer.parseInt(opt.get("param").toString())
                            ));
            ...
            // Cập nhật DB: trừ limit và ghi listUser
            if (limit != -1) {
                --limit;
            }
            listUser.add(p.id);
            GirlkunDB.executeUpdate("UPDATE `giftcode` SET `limit` = " + limit
                    + ", `listUser` = '" + listUser.toJSONString()
                    + "' WHERE `code` LIKE '" + Util.strSQL(giftcode) + "';");

            // Cập nhật client một lần duy nhất sau khi xử lý xong
            Service.getInstance().sendMoney(p);
            InventoryServiceNew.gI().sendItemBags(p);
            Service.gI().sendThongBaoOK(p, text);

        } catch (Exception e) {
            NpcService.gI().createTutorial(p, 30274, "Có lỗi xảy ra hãy báo ngay cho QTV để khắc phục.");
            e.printStackTrace();
        }
    }
}
