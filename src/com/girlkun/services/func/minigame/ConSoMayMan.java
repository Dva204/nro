...
//    public long giaSo_Gold = 1_000_000; // vàng
    ...
    public void addPlayerHongNgoc(Player pl) { // thêm player chơi = ngọc vô list
    ...
    public void removePlayerHongNgoc(Player pl) { // thêm player chơi = ngọc vô list
    ...
    public void newData(Player player, int point, int VangorNgoc) { // ngọc // vang = 0; ngoc = 1;
        ...
        Item hongngoc = InventoryServiceNew.gI().findItemBagByTemp(player, (short) 861);
        if (VangorNgoc == 0) {
            if (thoiVang == null) {
                Service.getInstance().sendThongBao(player, "Đéo có 1 Thỏi Vàng à????");
                return;
            ...
            if (hongngoc == null) {
                Service.getInstance().sendThongBao(player, "Đéo có nổi 5 Hồng Ngọc à????");
                return;
            } else if (hongngoc.quantity < this.giaSo_Ruby) {
                Service.getInstance().sendThongBao(player, "Đéo có nổi 5 Hồng Ngọc à????");
                return;
            }
        }
        ...
        if (VangorNgoc == 0) {
            if (!players.stream().anyMatch(game -> game.id == player.id && game.point == point)) {
                ...
                data.point = point;
                players.add(data);
                InventoryServiceNew.gI().subQuantityItemsBag(player, hongngoc, (int) giaSo_Ruby);
                InventoryServiceNew.gI().sendItemBags(player);
                Service.getInstance().sendThongBao(player, "Mày bị trừ " + hongngoc.quantity + " hồng ngọc vì cờ bạc");
        ...
        if (VangorNgoc == 1) {
            if (!players.stream().anyMatch(game -> game.id == player.id && game.point == point)) {
                ...
                InventoryServiceNew.gI().sendItemBags(player);
                Service.getInstance().sendThongBao(player, "Mày bị trừ " + thoiVang.quantity + " thỏi vàng vì cờ bạc");
    ...
    public void ramdom1SoLe(Player player, int VangorNgoc) { // ngọc
        ...
        if (VangorNgoc == 0) {
            ...
            if (hongngoc == null) {
                Service.getInstance().sendThongBao(player, "Bạn có hồng ngọc đéo đâu????");
                return;
            } else if (hongngoc.quantity < this.giaSo_Ruby) {
                Service.getInstance().sendThongBao(player, "Bạn có hồng ngọc đéo đâu????");
                return;
        ...
        if (VangorNgoc == 0) {
            if (!players.stream().anyMatch(game -> game.id == player.id && game.point == point)) {
                ...
                InventoryServiceNew.gI().subQuantityItemsBag(player, hongngoc, (int) giaSo_Ruby);
                InventoryServiceNew.gI().sendItemBags(player);
                Service.getInstance().sendThongBao(player, "Bạn bị trừ " + hongngoc.quantity + " hồng ngoc vì cờ bạc");
                player.inventory.ruby -= (giaSo_Ruby);
                Service.gI().sendMoney(player);
                Service.gI().showYourNumber(player, strNumber((int) player.id), null, null, 0);
            }
    ...
    public void ramdom1SoChan(Player player, int VangorNgoc) { // ngọc
        ...
        if (VangorNgoc == 0) {
            if (thoiVang == null) {
                Service.getInstance().sendThongBao(player, "Bạn có 1 hỏi vàng đéo đâu????");
                return;
            } else if (thoiVang.quantity < this.giaSo_Gold) {
                Service.getInstance().sendThongBao(player, "Bạn có đủ 1 thỏi vàng đéo đâu????");
                return;
            }
            //if (player.inventory.gold < 1_000_000) { // cái này là dùng để dùng vàng nhé thằng dev sau :)))
            //    Service.getInstance().sendThongBao(player, "Bạn không đủ 1 triệu vàng để thực hiện");
        ...
        if (VangorNgoc == 0) {
            if (!players.stream().anyMatch(game -> game.id == player.id && game.point == point)) {
                ...
                InventoryServiceNew.gI().subQuantityItemsBag(player, hongngoc, (int) giaSo_Ruby);
                InventoryServiceNew.gI().sendItemBags(player);
                Service.getInstance().sendThongBao(player, "Bạn bị trừ " + hongngoc.quantity + " hồng ngọc vì cờ bạc");
                player.inventory.ruby -= (giaSo_Ruby);
                Service.gI().sendMoney(player);
                Service.gI().showYourNumber(player, strNumber((int) player.id), null, null, 0);
            }
    ...
    public String strFinish(int id) {
        ...
        for (ConSoMayManData g : players) {
            if (id == g.id && g.point == result && g.conSoMayManNgoc == 1) {
                ...
                Item hongngoc = ItemService.gI().createNewItem((short) 861, (int) MiniGame.gI().MiniGame_S1.money);
                finish = "Chúc mừng " + (player == null ? "NULL" : player.name) + " đã thắng " + MiniGame.gI().MiniGame_S1.money + " hồng ngọc với con số may mắn " + result;
            ...
            if (id == g.id && g.point == result && g.conSoMayManVang == 1) {
                ...
                // trả thưởng cho player trúng thưởng //
                player.inventory.gold += MiniGame.gI().MiniGame_S1.gold;
                InventoryServiceNew.gI().sendItemBags(player);
                // end trả thưởng //
                break;
            }
