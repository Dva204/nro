...
public class Service {
    ...
    public void sendchienlinh(Player player, short smallId) {
        Message msg;
        try {
           // System.out.println("Gửi Chiến Linh cho player: " + player.name + ", smallId: " + smallId);
            msg = new Message(31);
            ...
            //System.out.println("Đã gửi Chiến Linh thành công cho player: " + player.name);
            msg.cleanup();
        } catch (Exception e) {
            System.out.println("Lỗi Send Chiến Linh cho player " + player.name + ": " + e.getMessage());
    ...
    /**
     * Gửi MỘT hoặc NHIỀU hiệu ứng danh hiệu, đồng thời xóa các hiệu ứng cũ đi.
     *
     * @param player Người chơi cần thêm hiệu ứng.
     * @param ids Danh sách các ID của danh hiệu cần hiển thị.
     */
    public void sendTitle(Player player, int... ids) {
        // BƯỚC 1: DỪNG VÒNG XOAY DANH HIỆU (NẾU ĐANG CHẠY)
        stopTitleRotation(player);

        // BƯỚC 2: GỬI LỆNH XÓA TẤT CẢ DANH HIỆU HIỆN CÓ
        try {
            Message msgXoa = new Message(-128);
            msgXoa.writer().writeByte(2); // Lệnh 2 là để xóa hiệu ứng
            msgXoa.writer().writeInt((int) player.id);
            this.sendMessAllPlayerInMap(player, msgXoa);
        } catch (Exception e) {
            e.printStackTrace();
        }

        // BƯỚC 3: HIỂN THỊ CÁC DANH HIỆU MỚI (NẾU CÓ)
        if (ids == null || ids.length == 0) {
            return; // Nếu không có ids nào thì chỉ thực hiện việc xóa
        }
        ...
        for (int id : ids) {
            ...
            try {
                ...
                // Logic map ID vào short value (giữ nguyên code của bạn)
                if (id == 891) {
                    me.writer().writeShort(2200);
                } else if (id == 889) {
                    me.writer().writeShort(2201);
                } else if (id == 890) {
                    me.writer().writeShort(2202);
                } else {
                    switch (id) {
                        case 1684:
                        ...
                        case 1993:
                            ...
                            break;
                        case 1994:
                            me.writer().writeShort(320);
                            break;
                        case 1995:
                            me.writer().writeShort(321);
                            break;
                        case 1893:
                            me.writer().writeShort(2207);
                            break;
                        case 1894:
                            me.writer().writeShort(2208);
                            break;
                        case 1896:
                            me.writer().writeShort(2209);
                            break;
                        case 1897:
                            me.writer().writeShort(2210);
                            break;
                        default:
                            if (me != null) {
                                me.cleanup();
    ...
    /**
     * Gửi MỘT hoặc NHIỀU hiệu ứng danh hiệu cho một người chơi khác (p2) thấy.
     *
     * @param player Người chơi "chủ" của hiệu ứng.
     * @param p2 Người chơi sẽ nhận và thấy hiệu ứng.
     * @param ids Danh sách các ID của danh hiệu cần hiển thị.
     */
    public void sendTitleRv(Player player, Player p2, int... ids) {
        // BƯỚC 1: XÓA HIỆU ỨNG CŨ TRÊN GÓC NHÌN CỦA P2
        try {
            Message msgXoa = new Message(-128);
            msgXoa.writer().writeByte(2);
            msgXoa.writer().writeInt((int) player.id);
            p2.sendMessage(msgXoa); // Chỉ gửi cho p2
            msgXoa.cleanup();
        } catch (Exception e) {
            e.printStackTrace();
        }

        // BƯỚC 2: GỬI CÁC HIỆU ỨNG MỚI CHO P2 XEM (NẾU CÓ)
        if (ids == null || ids.length == 0) {
            return;
        }
    ...
    public void reloadTitle(Player player) {
        ...
        Service.gI().stopTitleRotation(player);
        ...
        if (player.inventory.itemsBody.get(12).isNotNullItem() && player.inventory.itemsBody.get(12) != null && player.reloadtitle == true) {
        ...
        if (player.inventory.itemsBody.get(10).isNotNullItem() && player.inventory.itemsBody.get(10) != null) {
            Service.getInstance().sendchienlinh(player, (short) (player.inventory.itemsBody.get(10).template.id));
        }
...
//            e.printStackTrace();
    ...
    public void sendPetFollow(Player player, short smallId) {
        //System.out.println("Gửi PetFollow cho player: " + player.name + ", smallId: " + smallId);
        ...
        try {
            ...
            // System.out.println("Đã gửi PetFollow thành công!");
            msg.cleanup();
        } catch (Exception e) {
            //System.out.println("Lỗi Send Pet Follow: " + e.getMessage()); // Log chi tiết lỗi
    ...
    public void sendPetFollowToMe(Player me, Player pl) {
        ...
        Item linhThu = pl.inventory.itemsBody.get(7);
        if (!linhThu.isNotNullItem()) {
            return;
        }
    ...
    // Thêm 2 hàm mới này vào file Service.java
    /**
     * Bắt đầu tự động xoay vòng các danh hiệu cho người chơi.
     *
     * @param player Người chơi cần xoay danh hiệu.
     * @param titleIds Danh sách các ID danh hiệu để xoay.
     * @param intervalSeconds Thời gian (giây) giữa mỗi lần đổi.
     */
    public void startTitleRotation(Player player, java.util.List<Integer> titleIds, int intervalSeconds) {
        // Nếu đang có một vòng xoay cũ, hãy dừng nó lại trước
        stopTitleRotation(player);
        ...
        player.isTitleRotating = true;
        player.titleRotationTimer = new java.util.Timer();

        // Tạo một nhiệm vụ sẽ được lặp đi lặp lại
        java.util.TimerTask rotationTask = new java.util.TimerTask() {
            private int currentIndex = 0;

            @Override
            public void run() {
                // Nếu người chơi không còn online hoặc đã tắt chế độ, thì dừng lại
                if (player == null || !player.isTitleRotating || !com.girlkun.server.Client.gI().getPlayers().contains(player)) {
                    stopTitleRotation(player);
                    return;
                }

                // BƯỚC 1: XÓA DANH HIỆU HIỆN TẠI ĐI
                try {
                    Message msgXoa = new Message(-128);
                    msgXoa.writer().writeByte(2); // Lệnh 2 là xóa hiệu ứng danh hiệu
                    msgXoa.writer().writeInt((int) player.id);
                    sendMessAllPlayerInMap(player, msgXoa);
                    // cleanup() đã được gọi bên trong sendMessAllPlayerInMap
                } catch (Exception e) {
                    // Bỏ qua lỗi nếu có
                }

                // BƯỚC 2: GỬI DANH HIỆU MỚI TỚI
                int currentId = titleIds.get(currentIndex);
                Service.gI().sendTitle(player, currentId);

                // BƯỚC 3: CHUYỂN SANG INDEX TIẾP THEO
                currentIndex = (currentIndex + 1) % titleIds.size();
            }
        };

        // Lên lịch cho nhiệm vụ: chạy ngay lập tức, và sau đó lặp lại sau mỗi `intervalSeconds`
        player.titleRotationTimer.schedule(rotationTask, 0, intervalSeconds * 1000L);
    }

    /**
     * Dừng việc xoay danh hiệu.
     *
     * @param player Người chơi cần dừng.
     */
    public void stopTitleRotation(Player player) {
        if (player.isTitleRotating && player.titleRotationTimer != null) {
            player.titleRotationTimer.cancel();
            player.titleRotationTimer = null;
            player.isTitleRotating = false;

            // Gửi lệnh xóa hiệu ứng danh hiệu hiện tại
            Message me;
            try {
                me = new Message(-128);
                me.writer().writeByte(2); // Type 2 là xóa hiệu ứng
                me.writer().writeInt((int) player.id);
                sendMessAllPlayerInMap(player, me);
                me.cleanup();
            } catch (Exception e) {
                e.printStackTrace();
            }
    ...
    public void chat(Player player, String text) {
        // Thêm 2 lệnh này vào hàm chat() trong Service.java

// Lệnh để BẮT ĐẦU xoay danh hiệu
        if (text.equals("chuyendh")) {
            // Bạn có thể tùy chỉnh danh sách ID và thời gian ở đây
            java.util.List<Integer> ids = java.util.Arrays.asList(1986, 1987, 1988, 1989, 1994, 1995);
            int thoiGianGiay = 3; // Cứ 3 giây đổi 1 lần

            Service.gI().startTitleRotation(player, ids, thoiGianGiay);
            Service.gI().sendThongBao(player, "Đã bật tự động chuyển danh hiệu!");
            return;
        }

// Lệnh để DỪNG xoay danh hiệu
        if (text.equals("dungdh")) {
            Service.gI().stopTitleRotation(player);
            Service.gI().sendThongBao(player, "Đã tắt tự động chuyển danh hiệu!");
            return;
        }
...
//                }
            ...
            if (text.equals("info")) {
                NpcService.gI().createMenuConMeo(player, ConstNpc.MENU_PLAYER, 30274,
                        ""
                        + "|4| THÔNG TIN NHÂN VẬT NGỌC RỒNG AS\n"
...
//                }
            ...
            if (text.equals("ad")) {
                NpcService.gI().createMenuConMeo(player, ConstNpc.MENU_ADMIN, 30274, "|7| KÝ SỰ NGỌC RỒNG\b|4| Số Người Online: " + Client.gI().getPlayers().size() + "\n" + "|8|Current Thread: " + (Thread.activeCount() - ServerManager.gI().threadMap)
...
//                }
                ...
                try {
                    ...
                    if (Short.parseShort(item[0]) <= 9999) {
...
//                }
        ...
        if (text.equals("banv")) {
            ...
            if (now >= lasttimechatbanv + 10000) {
                ...
                Service.getInstance().sendThongBao(player, "Spam CON CỤ MÀY !");
                return;
            }
        }
...
//        }
        if (text.equals("boss")) {
            BossManager.gI().showListBossClient(player);
            return;
        }
    ...
    public void player(Player pl) {
        ...
        try {
            ...
            msg.writer().writeShort(30274); //char info id
...
//            Send_Info_NV(pl);
    ...
    public void sendThongBaoFromAdmin(Player player, String text) {
        sendBigMessage(player, 30274, text);
    }
...
//            System.out.println(platform);
    ...
    public void sendUpdateSkill(Player player) {
