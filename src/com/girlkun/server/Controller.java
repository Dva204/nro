...
//            System.out.println("***************************CMD receive: " + cmd);
            switch (cmd) {
                ...
                case -113:
                    if (player != null) {
                    player.playerSkill.skillShortCut.clear(); // Xóa danh sách cũ
                    for (int i = 0; i < 10; i++) {
                    byte shortcut = _msg.reader().readByte();
                     player.playerSkill.skillShortCut.add(shortcut);
                    }
        ...
        if (msg != null) {
            try {
                 if (true) {
                    ...
                    if (!key.equals("sljkfjdsfoewoipiut[goigpob[dikgoiof[gưeprgiopdfgidfpigdf")) {
                        Service.getInstance().sendThongBaoOK(session, "Vui lòng tải phiên bản tại trang chủ Nro AS.");
                        session.disconnect();
                        return;
                    }
                }
    ...
    public void sendInfo(MySession session) {
        ...
        if (TaskService.gI().getIdTask(player) == ConstTask.TASK_0_0) {
            ...
            NpcService.gI().createTutorial(player, -1,
                    "Chào mừng " + player.name + " đến với Ngọc Rồng As\n"
...
//    }
    ...
    private void sendThongBaoServer(Player player) {
    Service.gI().sendThongBaoFromAdmin(player, "|7| Chào Mừng " + player.name + " Đến Với Ngọc Rống As\n|5| Website : asnro.com\n|3| Mọi thắc mắc vui lòng liên hệ ADMIN để được giải đáp !\b|2| Chúc Bạn Chơi Game Vui Vẻ... " );
    ItemTimeService.gI().sendTextTime(player, (byte) KHI_GASS, "Mỗi ngày điểm danh nhận Thỏi Vàng ", 5);
    ItemTimeService.gI().sendTextTime(player, (byte) BAN_DO_KHO_BAU, "Nhớ muốn tăng tiến sức mạnh nhanh cần đọc cẩm nang ở mục hướng dẫn trên website ", 10);
    ItemTimeService.gI().sendTextTime(player, (byte) DOANH_TRAI, "Donate Cho ADMIN để trải nghiệm làm Rich nhé", 15);
    ItemTimeService.gI().sendTextTime(player, (byte) CON_DUONG_RAN_DOC, "Mọi thắc mắc liên hệ ADMIN để được hỗ trợ ", 20);
           if (Util.canDoWithTime(this.lastTimeGK, 15000)) {
                    this.lastTimeGK = System.currentTimeMillis();
                    Service.gI().sendThongBaoAllPlayer("|2|Chào mừng bạn đã đến với Ngọc Rồng As");
    }
    }
