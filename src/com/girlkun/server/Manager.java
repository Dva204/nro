...
public class Manager {
    ...
    public static final String queryTopSM = "SELECT id, CAST(JSON_EXTRACT(data_point, '$[1]') AS UNSIGNED) AS sm FROM player ORDER BY sm DESC LIMIT 10;";
    ...
    public static final String queryTopNV = "SELECT id, "
            + "CAST(JSON_EXTRACT(data_task, '$[0]') AS UNSIGNED) AS nv_main_id, "
            + "CAST(JSON_EXTRACT(data_task, '$[1]') AS UNSIGNED) AS nv_sub_idx, "
            + "CAST(JSON_EXTRACT(data_task, '$[2]') AS UNSIGNED) AS nv_count "
            + "FROM player "
            + "ORDER BY nv_main_id DESC, nv_sub_idx DESC, nv_count DESC "
            + "LIMIT 20;";
    ...
    public static final byte[] itemIds_NR_SB = {14, 15, 16};//1s,2s,3s
    public static final byte[] itemIds_NR_SB23 = {15, 16};//1s,2s,3s
    ...
    public static final short[] itemDC13 = {555, 556, 557, 558, 559, 560, 561, 562, 563, 564, 565, 566, 567};
    public static final short[] itemDC14 = {650, 651, 652, 653, 654, 655, 656, 657, 658, 659, 660, 661, 662};
    ...
    private void loadDatabase() {
        ...
        try (Connection con = GirlkunDB.getConnection();) {
            ...
            while (rs.next()) {
                ...
                //itemTemp.description = (desc != null) ? desc : "";
...
//                RUBY_REWARDS.add(Util.sendDo(861, 0, new ArrayList<>()));
            ...
            while (rs.next()) {
                ...
                switch (query) {
                    case queryTopSM:
                        ...
                        break;
                    case queryTopNV: {
                        int nvMainId = rs.getInt("nv_main_id");
                        int nvSubIdx = rs.getInt("nv_sub_idx");
                        String tenSub = "Bước " + nvSubIdx;
                        for (TaskMain task : Manager.TASKS) {
                            if (task.id == nvMainId) {
                                if (nvSubIdx >= 0 && nvSubIdx < task.subTasks.size()) {
                                    String raw = task.subTasks.get(nvSubIdx).name;
                                    // Thay %N bằng chuỗi rỗng cho gọn
                                    raw = raw.replaceAll("%\\d+", "").trim();
                                    // Cắt ngắn nếu quá 22 ký tự
                                    tenSub = raw.length() > 22 ? raw.substring(0, 21) + "…" : raw;
                                }
                                break;
                            }
                        }
                        top.setInfo1("NV" + nvMainId + ": " + tenSub);
                        top.setInfo2("Bảng Xếp Hạng Nhiệm Vụ");
                        break;
                    }
                    case queryTopSK:
                        top.setInfo1(Util.numberToMoney(rs.getLong("event")) + " điểm");
                        top.setInfo2("Bảng Xếp Hạng SỰ KIỆN");
                        break;
                    case queryTopPVP:
                        top.setInfo1(Util.numberToMoney(rs.getLong("pointPvp")) + " điểm");
                        top.setInfo2("Bảng Xếp Hạng PVP");
                        break;
                    case queryTopNHS:
                        top.setInfo1(Util.numberToMoney(rs.getLong("NguHanhSonPoin")) + " điểm");
                        top.setInfo2("Bảng Xếp Hạng Ngũ Hành Sơn");
                        break;
                    case queryTopNap:
                        top.setInfo1(String.format("%,d", rs.getLong("tongnap")) + " VND");
                        top.setInfo2("Bảng Xếp Hạng NẠP");
                        break;
                    case queryTopCS:
                        top.setInfo1(Util.numberToMoney(rs.getLong("chuyenSinh")) + " lần");
                        top.setInfo2("Bảng Xếp Hạng Chuyển Sinh");
                        break;
                    case queryTopGapThu:
                        top.setInfo1(Util.numberToMoney(rs.getLong("p_thu")) + " lần gắp");
                        top.setInfo2("Bảng Xếp Hạng Gắp Thú");
                        break;
                    case queryTopLeoThap:
                        top.setInfo1(rs.getLong("leothap") + " tầng");
                        top.setInfo2("Bảng Xếp Hạng Leo Tháp");
                        break;
                    case queryTopThachDau:
                        top.setInfo1("Level " + rs.getLong("thachdauwhis"));
                        top.setInfo2("Bảng Xếp Hạng Thách Đấu");
                        break;
                    case queryTopCauCa:
                        top.setInfo1(Util.numberToMoney(rs.getLong("cauca")) + " điểm");
                        top.setInfo2("Bảng Xếp Hạng Câu Cá");
                        break;
                    case queryTopSD:
                        top.setInfo1(Util.numberToMoney(rs.getLong("sd")) + " sức đánh");
    ...
    public void loadProperties() throws IOException {
        ...
        Object value = null;
        //###Config sv
        if ((value = properties.get("server.lee.port")) != null) {
            ServerManager.PORT = Integer.parseInt(String.valueOf(value));
        }
        if ((value = properties.get("server.lee.name")) != null) {
            ServerManager.NAME = String.valueOf(value);
        }
        if ((value = properties.get("server.lee.sv")) != null) {
        ...
        // ===================================================================
        // Chỉ gọi substring (để xóa dấu phẩy cuối) khi chuỗi linkServer không rỗng
        if (linkServer.length() > 0) {
            DataGame.LINK_IP_PORT = linkServer.substring(0, linkServer.length() - 1);
        } else {
            // Nếu không tìm thấy server nào, gán nó là chuỗi rỗng để tránh lỗi
            DataGame.LINK_IP_PORT = "";
        }

        // ===================================================================
        // KẾT THÚC SỬA LỖI
        // ===================================================================
        if ((value = properties.get("server.lee.waitlogin")) != null) {
            SECOND_WAIT_LOGIN = Byte.parseByte(String.valueOf(value));
        }
        if ((value = properties.get("server.lee.maxperip")) != null) {
            MAX_PER_IP = Integer.parseInt(String.valueOf(value));
        }
        if ((value = properties.get("server.lee.maxplayer")) != null) {
            MAX_PLAYER = Integer.parseInt(String.valueOf(value));
        }
        if ((value = properties.get("server.lee.expserver")) != null) {
            RATE_EXP_SERVER = Byte.parseByte(String.valueOf(value));
        }
        if ((value = properties.get("server.lee.local")) != null) {
