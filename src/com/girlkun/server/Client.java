...
//        }
        while (!players.isEmpty()) {
            ...
            if (p != null && p.getSession() instanceof MySession) {
                this.kickSession((MySession) p.getSession());
            }
        }
    ...
    public void cloneMySessionNotConnect() {
        ...
        if (!GirlkunSessionManager.gI().getSessions().isEmpty()) {
            for (int j = 0; j < GirlkunSessionManager.gI().getSessions().size(); j++) {
                ...
                j--; // Giảm index do đã remove phần tử
    ...
    public void kickSessionByIp(String ip) {
        for (MySession session : getSessions()) {
