...
public class SkillUtil {
    ...
    for (NClass nClass : Manager.NCLASS) {
        SkillTemplate temp = nClass.getSkillTemplate(tempId);
        if (temp != null) {
            return new Skill(temp.skillss.get(level - 1));
        }
    }
    return createEmptySkill();
}
    
    public static Skill createSkillForPet(int tempId, int level) {
    for (NClass nClass : Manager.NCLASS) {
        SkillTemplate temp = nClass.getSkillTemplate(tempId);
        if (temp != null && temp.skillss.size() >= level) {
            return new Skill(temp.skillss.get(level - 1));
        }
    }
    return createEmptySkill();
