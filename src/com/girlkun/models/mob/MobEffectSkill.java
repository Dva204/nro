...
public class MobEffectSkill {
    ...
    public void setStun(Player plUseSkill, long lastTimeStun, int timeStun) {
    this.lastTimeStun = lastTimeStun;
    this.timeStun = timeStun;
    this.isStun = true;
    try {
        Message msg = new Message(-124);
        msg.writer().writeByte(0); // b1
        msg.writer().writeByte(1); // b2
        msg.writer().writeByte(40); // effect type = choáng
        msg.writer().writeByte(mob.id); // id mob bị choáng
