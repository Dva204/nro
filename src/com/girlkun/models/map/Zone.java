            ...
            if (this.map.mapId == 5) {
                ...
                for (Npc npc : this.map.npcs) {
                    if (npc.tempId == ConstNpc.QUY_LAO_KAME) {
                        npc.npcChat("Hello tất cả các tình yêu");
                        System.err.println("Chào mừng các tình yêu đến với Ngọc Rồng As ");
                    }
    ...
    public List<ItemMap> getItemMapsForPlayer(Player player) {
        ...
        for (ItemMap item : items) {
            if (item == null || item.itemTemplate == null) {
                continue; // bỏ qua item null hoặc itemTemplate null
            }
    ...
    public void load_Another_To_Me(Player player) { //load những player trong map và gửi cho player vào map
        try {
            if (MapService.gI().isMapOffline(this.map.mapId)) {
                ...
                for (Player pl : new ArrayList<>(this.humanoids)) {
...
//            Logger.logException(MapService.class, e);
        ...
        if (!plInfo.isBoss && !plInfo.isPet && !plInfo.isNewPet && !plInfo.isPet3 && !(plInfo instanceof BossDHVT) && !(plInfo instanceof Referee) && !(plInfo instanceof BossVD) && !(plInfo instanceof Referee1) && !(plInfo instanceof TestDame) && !(plInfo instanceof Referee2)) {
            ...
            //if (plInfo.inventory.itemsBody.get(13).isNotNullItem()) { // Hoặc điều kiện kiểm tra chiến linh của bạn
            //Service.getInstance().sendchienlinh(plInfo, plReceive, (short) plInfo.inventory.itemsBody.get(13).template.id);
            //}
            ...
            if (plInfo.inventory.itemsBody.get(10).isNotNullItem()) {
                Service.gI().sendTitleRv(plInfo, plReceive, (short) (plInfo.inventory.itemsBody.get(10).template.id));
            }
