...
public class ShopServiceNew {
    ...
    public void opendShop(Player player, String tagName, boolean allGender) {
        if (tagName.equals("ITEMS_LUCKY_ROUND")) {
            ...
            openShopType4(player, tagName, player.getSession().itemsReward);
            return;
        }
        try {
    ...
    public void takeItem(Player player, byte type, int tempId) {
        ...
        if (tagName.equals("ITEMS_LUCKY_ROUND")) {
            ...
            getItemSideBoxLuckyRound(player, player.getSession().itemsReward, type, tempId);
        ...
        switch (itS.template.id) {
            ...
            case 457:
                if (itemShop.tabShop.shop.tagName.equals("BILL")) {
                    ...
                    Item specItem = InventoryServiceNew.gI().findItemBag(pl, itSpec);

                    if (specItem == null || specItem.quantity < buySpec) {
                        Service.getInstance().sendThongBao(pl, "Không đủ " + buySpec + " " + itS.template.name);
                        return false;
                    }

                    for (Item i : pl.inventory.itemsBag) {
                        if (i.template != null && ItemData.list_thuc_an.contains((int) i.template.id)) {

                            if (i.quantity >= 99) {

                                InventoryServiceNew.gI().subQuantityItemsBag(pl, i, 99);
                                InventoryServiceNew.gI().subQuantityItemsBag(pl, specItem, buySpec);
