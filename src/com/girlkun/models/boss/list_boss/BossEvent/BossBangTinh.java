...
public class BossBangTinh extends Boss {
    ...
    public void reward(Player plKill) {
        ...
        int a = 0;
        for (int i = 0; i < 10; i++) {
            ItemMap it1 = new ItemMap(this.zone, 1613, 1, this.location.x + a, this.zone.map.yPhysicInTop(this.location.x,
        ...
        for (int i = 0; i < 5; i++) {
            ItemMap it2 = new ItemMap(this.zone, 1614, 1, this.location.x + a, this.zone.map.yPhysicInTop(this.location.x,
                    this.location.y - 24), -1);
        ...
        for (int i = 0; i < this.PlayerPlAtt.size(); i++) {
            ...
            if (pl != null && pl.zone != null && pl.zone.map.mapId == 184
                ...
                if (ptnhanqua >= 3 && ptnhanqua <= 3) {
                    ...
                    Item Duoikhi = new Item();
                    Duoikhi.template = ItemService.gI().getTemplate(674);
                    Duoikhi.quantity = 10;
                    InventoryServiceNew.gI().addItemBag(pl, Duoikhi);
                    InventoryServiceNew.gI().sendItemBags(pl);
                    Service.gI().sendThongBaoOK(pl, "Bạn nhận được 10 đá ngũ sắc khi tham gia gây dame BOSS ");
                    pl.DucNTdamethanmeo = 0;
