    ...
    public void reward(Player plKill) {
        if (this.gethour() >= 19 && this.gethour() <= 22) { // Sửa ở đây
            ...
            if (plKill != null) {
                ...
                Item KHTDkill = new Item();
                tvkill.template = ItemService.gI().getTemplate(2000);
                tvkill.quantity = 1;
                InventoryServiceNew.gI().addItemBag(plKill, KHTDkill);
                InventoryServiceNew.gI().sendItemBags(plKill);
                Service.gI().sendThongBaoOK(plKill,
                        "Bạn đã gây " + this.df(ptnhanquaKill) + "% HP boss\n Bạn nhận được:\n"
                        + this.df(ptnhanquaKill * 10) + " thỏi vàng\n" + "\nBạn nhận đc 20K Hồng ngọc và 1 Hòm Kích Hoạt Trái Đất do kết liễu Boss.");
