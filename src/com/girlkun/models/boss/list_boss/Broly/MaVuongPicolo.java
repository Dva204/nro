...
public class MaVuongPicolo extends Boss {
    ...
    public String df(double sfm) {
     ...
     public void reward(Player plKill) {
        if (this.gethour() >= 19 && this.gethour() <= 22) { // Sửa ở đây

          //  int a = 0;
          //  for (int i = 0; i < 20; i++) {
          //      ItemMap it1 = new ItemMap(this.zone, 2002, 1, this.location.x + a, this.zone.map.yPhysicInTop(this.location.x,
          ...
          //  }
            ...
            if (plKill != null) {
                ...
                //capsuleTT.quantity = 1;
                //InventoryServiceNew.gI().addItemBag(plKill, capsuleTT);
                Item KHNM = new Item();
                KHNM.template = ItemService.gI().getTemplate(2001);
                KHNM.quantity = 1;
                InventoryServiceNew.gI().addItemBag(plKill, KHNM);
                InventoryServiceNew.gI().sendItemBags(plKill);
                Service.gI().sendThongBaoOK(plKill, "\nBạn nhận được 20K Hồng ngọc và 1 Hộp Trang Bị Kích Hoạt Namec do kết liễu Boss.");
...
//                        InventoryServiceNew.gI().sendItemBags(pl);
                        Service.gI().sendThongBaoOK(pl, "Bạn nhận được 20 Thỏi Vàng thưởng tham gia sự kiện ");
                        pl.DucNTdamethanmeo = 0;
...
//    @Override
    public void attack() {
        if (Util.canDoWithTime(this.lastTimeAttack, 100) && this.typePk == ConstPlayer.PK_ALL
                && (this.gethour() >= 19 && this.gethour() <= 22)) { // Sửa ở đây
