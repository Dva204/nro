...
public class Xuka extends Boss {
    ...
    public void reward(Player plKill) {
        ...
        if (Util.isTrue(BossManager.ratioReward, 100)) {
            ...
            Service.gI().dropItemMap(this.zone, Util.RaitiDoc12(zone, Manager.itemDC13[randomDo], 1, this.location.x, this.location.y, plKill.id));
    ...
    public double injured(Player plAtt, double damage, boolean piercing, boolean isMobAttack) {
        ...
        if (isDie()) {
            this.setDie(plAtt);
            super.die(plAtt); // gọi hàm chết cha
            TaskService.gI().checkDoneTaskKillBoss(plAtt, this); // gọi nhiệm vụ kill boss
        }
        return damage;
        } else {
            return 0;
