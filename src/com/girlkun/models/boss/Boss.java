...
public class Boss extends Player implements IBossNew, IBossOutfit {
    ...
    protected void notifyJoinMap() {
        ...
        // Thông báo lúc boss vừa xuất hiện
        ServerNotify.gI().notify("BOSS " + this.name + " vừa xuất hiện tại " + this.zone.map.mapName);

        // Vòng lặp gửi lại thông báo mỗi 30s
//        new Thread(() -> {
//            while (true) {
//                try {
//                    if (this.zone != null && !this.isDie()) {
//                        ServerNotify.gI().notify("BOSS " + this.name + " đang ở " + this.zone.map.mapName);
//                    } else {
//                        break;
//                    }
//                    Thread.sleep(10_000);
...
//                Logger.logException(Boss.class, ex);
    ...
    public void wakeupAnotherBossWhenAppear() {
        ...
        for (Boss boss : this.bossAppearTogether[this.currentLevel]) {
            ...
            if (boss.data[nextLevelBoss].getTypeAppear() == TypeAppear.ANOTHER_LEVEL) {
                if (boss.zone != null) {
                    boss.leaveMap();
                }
                boss.changeStatus(BossStatus.RESPAWN);
