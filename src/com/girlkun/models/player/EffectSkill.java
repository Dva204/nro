...
//    public int timeAnTroi;
    ...
    public boolean isCadicMonkeyBonus;
    ...
    if (player.nPoint.khangTDHS) {
        EffectSkillService.gI().removeStun(this.player);
    } else if (Util.canDoWithTime(lastTimeStartStun, timeStun) || player.isDie()) {
        EffectSkillService.gI().removeStun(this.player);
    }
}
...
//        }
   ...
   // this.isStun = true;
   // this.lastTimeStartStun = System.currentTimeMillis();
   // this.timeStun = time;
     EffectSkillService.gI().startStun(this.player, System.currentTimeMillis(), time);
}
}
