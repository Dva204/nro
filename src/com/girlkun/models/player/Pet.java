        ...
        try {
            ...
            switch (this.playerSkill.skillSelect.template.id) {
                ...
                case Skill.KAIOKEN:
                    ...
                    return false;
                case Skill.LIEN_HOAN: // thêm xử lý giống Kaioken
                    if (SkillService.gI().canUseSkillWithCooldown(this)) {
                        mobAttack = this.findMobAttack();
                        if (mobAttack == null) {
                            return false;
...
//========================BETA SKILL5=====================
    ...
    private void increasePoint() {
        if (this.nPoint != null && Util.canDoWithTime(lastTimeIncreasePoint, 1)) {
            if (Util.isTrue(50, 100)) {
                this.nPoint.increasePoint((byte) 2, (short) Util.nextInt(1, 1000));
            } else {
                byte type = (byte) Util.nextInt(0, 2);
                short point = (short) Util.nextInt(10000);
    ...
    private void followMaster(int dis) {
        // Khoảng cách tối đa pet đứng xa sư phụ (lấy từ `dis` truyền vào, đang là 40)
        int MAX_DISTANCE_FOLLOW = dis;

        // ***BỔ SUNG: Khoảng cách tối thiểu pet phải đứng (sửa số 60 nếu muốn)***
        int MIN_DISTANCE_STAND = 60;
        ...
        // Nếu pet ở xa hơn khoảng cách tối đa -> di chuyển lại gần
        if (distance >= MAX_DISTANCE_FOLLOW) {
            int disX = this.location.x - mX;
            if (disX < 0) {
                this.location.x = mX - Util.nextInt(0, MAX_DISTANCE_FOLLOW);
            } else {
                this.location.x = mX + Util.nextInt(0, MAX_DISTANCE_FOLLOW);
            }
            this.location.y = mY;
            PlayerService.gI().playerMove(this, this.location.x, this.location.y);
        } // ***THÊM LOGIC: Nếu pet ở gần hơn khoảng cách tối thiểu -> lùi ra xa***
        else if (distance < MIN_DISTANCE_STAND) {
            int disX = this.location.x - mX;
            int moveX = (disX < 0 ? -1 : 1) * 5; // Lùi 5 đơn vị

            this.location.x += moveX;
            this.location.y = mY;
            PlayerService.gI().playerMove(this, this.location.x, this.location.y);
        }
        // Nếu pet ở trong khoảng (MIN_DISTANCE_STAND, MAX_DISTANCE_FOLLOW) -> đứng yên
    }
    ...
    public void openSkill1() {
        Skill skill = SkillUtil.createSkill(Skill.LIEN_HOAN, 1);
        if (skill != null) {
            skill.coolDown = 1000;
        ...
        // ép client cập nhật skill mới
        try {
            Player player = this.playerSkill.player;
            Service.gI().sendUpdateSkill(player);
        } catch (Exception e) {
            e.printStackTrace();
        }

        System.out.println("Pet mở skill1: " + skill.template.name + " (id=" + skill.template.id + ")");
    }
    ...
    public void openSkill3() {
        ...
        } else if (rd <= tiLeTDHS + tiLeTTNL + tiLeLienHoan) {
            skill = SkillUtil.createSkill(Skill.LIEN_HOAN, 1);
        } else { // còn lại là Kaioken
