...
//    public PlayerEvent event;
    ...
    public boolean isTitleRotating = false;
    public java.util.Timer titleRotationTimer;
    ...
    public long lastTimePrepareTuSat = 0;
    ...
    public thanlinhClothers thanlinhClothers;
            ...
            try {
                if (!iDMark.isBan()) {
                    ...
                    if (this.isPl() && this.setClothes.ocTieu == 5) {
                        Service.gI().sendTitle(this, 1990);
                    }
                    if (this.isPl() && this.setClothes.picolo == 5) {
                        Service.gI().sendTitle(this, 1989);
                    }
                    if (this.isPl() && this.setClothes.pikkoroDaimao == 5) {
                        Service.gI().sendTitle(this, 1988);
                    }
                    if (this.isPl() && this.setClothes.nappa == 5) {
                        Service.gI().sendTitle(this, 1985);
                    }
                    if (this.isPl() && this.setClothes.cadic == 5) {
                        Service.gI().sendTitle(this, 1987);
                    }
                    if (this.isPl() && this.setClothes.kakarot == 5) {
                        Service.gI().sendTitle(this, 1984);
                    }
                    if (this.isPl() && this.setClothes.songoku == 5) {
                        Service.gI().sendTitle(this, 1994);
                    }
                    if (this.isPl() && this.setClothes.kirin == 5) {
                        Service.gI().sendTitle(this, 1993);
                    }
                    if (this.isPl() && this.setClothes.kaioken == 5) {
                        Service.gI().sendTitle(this, 1992);
                    }
                    if (this.isPl() && this.setClothes.SetTinhAn == 5) {
                        // Service.gI().sendTitle(this, 889);
                    }
                    if (this.isPl() && this.setClothes.SetNhatAn == 5) {
                        //Service.gI().sendTitle(this, 889);
                    }
                    if (this.isPl() && this.setClothes.SetNguyetAn == 5) {
                        //Service.gI().sendTitle(this, 889);
                    }
                    if (isPl()) {
                    }
                    ...
                    if (this.isPl() && this.inventory.itemsBody.size() > 7 && this.inventory.itemsBody.get(7) != null) {
                    ...
                    } else if (this.isPl() && newpet != null && (this.inventory.itemsBody.size() <= 7 || !this.inventory.itemsBody.get(7).isNotNullItem())) {
                        newpet.dispose();
                        newpet = null;
                    }
...
//    }
    ...
    private static final short[][] idOutfitFusion = {
        {1844, 1845, 1846}, {1549, 1554, 1555}, {1601, 1605, 1606},
        {1805, 1806, 1807}, {1599, 1605, 1606}, {1556, 1561, 1562},
        {1817, 1818, 1819}, {1717, 1718, 1719}, {1841, 1842, 1843},
        {1928, 1930, 1931}, {1924, 1926, 1927}, {1932, 1934, 1935},};
...
//                }
    ...
    public byte getAura() {
        ...
        // ====== SỬA LỖI LOGIC LẤY AURA TỪ CARD ======
        try {
            if (this.isPl() && this.Cards != null && !this.Cards.isEmpty()) {
                // 1. Tìm thẻ đang được SỬ DỤNG (Used == 1)
                Card usedCard = this.Cards.stream().filter(c -> c.Used == 1).findFirst().orElse(null);

                if (usedCard != null) {
                    // 2. Lấy thông tin thẻ MẪU (template)
                    RadarCard radarTemplate = RadarService.gI().RADAR_TEMPLATE
                            .stream()
                            .filter(r -> r.Id == usedCard.Id)
                            .findFirst()
                            .orElse(null);

                    // 3. Kiểm tra xem thẻ mẫu này có AuraId không (-1 là không có)
                    if (radarTemplate != null && radarTemplate.AuraId != -1) {
                        return (byte) radarTemplate.AuraId; // Trả về ID hào quang
                    }
                }
            }
        } catch (Exception e) {
            // Lỗi thì bỏ qua, chạy tiếp logic bên dưới
        }
        // ====== KẾT THÚC SỬA LỖI ======

        // Nếu không đủ slot đồ
...
//        PVPServcice.gI().finishPVP(this, PVP.TYPE_DIE);
    ...
    public class thanlinhClothers {

        //public int setTL;
        //public int setGod;
        //public int setGod;
        public thanlinhClothers() {
        }
    }
}
